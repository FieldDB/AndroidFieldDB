...
public class RegisterUserService extends NotifyingIntentService {
  ...
  protected void onHandleIntent(Intent intent) {
    ...
    com.github.fielddb.model.Activity.sendActivity("{\"register\" : \"username\"}", "{}",
        "*** Registered user ssucessfully ***");
  }
  ...
  public String registerUsers(Uri uri) {
    String[] userProjection = { UserTable.COLUMN_REV, UserTable.COLUMN_USERNAME, UserTable.COLUMN_FIRSTNAME,
        ...
        UserTable.COLUMN_GENERATED_PASSWORD, UserTable.COLUMN_APP_VERSIONS_WHEN_MODIFIED };
    if (uri == null) {
      uri = FieldDBUserContentProvider.CONTENT_URI;
    }
    CursorLoader cursorLoader = new CursorLoader(getApplicationContext(), uri, userProjection, null, null, null);
    Cursor cursor = cursorLoader.loadInBackground();
    if (cursor == null) {
      Log.e(Config.TAG, "There is no user... this is not supposed to happen.");
      return null;
    }
    cursor.moveToFirst();
    ...
    String appVersionsWhenModified = cursor.getString(cursor
    ...
    jsonParam.addProperty("authUrl", Config.DEFAULT_REGISTER_USER_URL.replace("/register", ""));
    jsonParam.addProperty("appVersionsWhenModified", appVersionsWhenModified);
    ...
    if (JSONResponse.contains("name already exists")) {
      JSONResponse = this.loginUser(username, generatedPassword, Config.DEFAULT_AUTH_LOGIN_URL);
    }
    if (!"".equals(this.userFriendlyErrorMessage)) {
      return null;
    }
  ...
  public int processRegistrationResponse(Uri uri, String jsonResponse) {
    ...
    try {
      values.put(UserTable.COLUMN_USERNAME, json.get("username").getAsString());
      values.put(UserTable.COLUMN_ID, json.get("_id").getAsString());
      values.put(UserTable.COLUMN_REV, json.get("_rev").getAsString());
      values.put(UserTable.COLUMN_GRAVATAR, json.get("gravatar").getAsString());
    } catch (Exception e) {
