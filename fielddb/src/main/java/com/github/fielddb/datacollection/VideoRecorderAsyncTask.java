...
public class VideoRecorderAsyncTask extends AsyncTask<Void, Void, String> {
  ...
  public static Camera getCameraInstance() throws IOException {
    ...
    if (sdk < 9) {
      ...
      Log.d(Config.TAG, "Trying any camera.");
      try {
        c = Camera.open();
      } catch (Exception e) {
        Log.d(Config.TAG, "Any Camera failed.");
      }
      if (c == null) {
    ...
    if (c == null) {
      ...
      Log.d(Config.TAG, "Trying camera 1.");
      try {
        c = Camera.open(1);
      } catch (Exception e) {
        Log.d(Config.TAG, "Camera 1 failed.");
      }
      cameraNumberUsed = 1;
      if (c == null) {
        /* Nexus 7 */
        Log.d(Config.TAG, "Trying camera 0.");
        try {
          c = Camera.open(0);
        } catch (Exception e) {
          Log.d(Config.TAG, "Camera 1 failed.");
  ...
  protected void beginRecording(SurfaceHolder holder) throws IOException {
    ...
    try {
      ...
      try {
        ...
        Log.d(Config.TAG, "There was a problem preparing the video recorder.");
      }
      Thread.sleep(500);
  ...
  public void beginRecordingAudio() {
    ...
    Log.e(Config.TAG, "Recording audio instead " + this.mVideoResultsFile);
  }
  ...
  protected void onPreExecute() {
    ...
    if (this.mVideoResultsFile == null || "".equals(this.mVideoResultsFile)) {
      mVideoResultsFile = Config.DEFAULT_OUTPUT_DIRECTORY + "/video/result_file_"
